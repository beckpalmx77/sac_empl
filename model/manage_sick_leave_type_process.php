...
## Total number of records without filtering
    ...
    $sql_cond = "SELECT COUNT(*) AS allcount FROM mleave_type WHERE day_flag = 'L' AND leave_type_id <> 'L2' ";
...
## Total number of records with filtering
    $stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM mleave_type WHERE leave_type_id <> 'L2' " . $searchQuery);
    $stmt->execute($searchArray);
    $records = $stmt->fetch();
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $stmt = $conn->prepare("SELECT * FROM mleave_type WHERE leave_type_id <> 'L2' " . $searchQuery
